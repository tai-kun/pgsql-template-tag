...
describe("基本的なインスタンス化とプレースホルダ生成の振る舞い", () => {
  test("1 つのバインド変数を持つ SQL を生成したとき、プレースホルダ $1 と値が正しく設定される", ({
    expect,
  }) => {
    // Arrange
  ...
  test("複数の異なるバインド変数を持つ SQL を生成したとき、インクリメントされたプレースホルダが割り振られる", ({
    expect,
  }) => {
    // Arrange
  ...
  test("10 個以上のバインド変数があるとき、2 桁のプレースホルダ ID が正しく採番される", ({
    expect,
  }) => {
    // Arrange
    const count = 11;
...
describe("変数の重複排除の振る舞い", () => {
  test("同一の値が複数回バインドされたとき、同じプレースホルダ ID が再利用され values が圧縮される", ({
    expect,
  }) => {
    // Arrange
...
describe("ユーティリティ関数の振る舞い", () => {
  ...
  test("raw 関数を使用したとき、プレースホルダ化されずに文字列がそのまま保持される", ({
    expect,
  }) => {
    // Act
